...
public class UserTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        String authHeader = request.getHeader(userToken.getHeader());
        if (authHeader != null && authHeader.startsWith(userToken.getTokenHead())) {
            final String authToken = authHeader.substring(userToken.getTokenHead().length());
            String username = userHelper.getUsernameFromToken(authToken);
            logger.info("checking authentication " + username);
            if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                JwtUser userDetails = (JwtUser)this.userDetailsServiceImpl.loadUserByUsername(username);
